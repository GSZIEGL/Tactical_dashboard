        ...
        rows.append(
            {
                ...
                "marker_size": 320 if code in [selected_a, selected_b] else 260,
...
def render_strategy_map(selected_a: Optional[str] = None, selected_b: Optional[str] = None):
    ...
    spec = {
        ...
        "layer": [
            ...
            {
                "mark": {"type": "text", "dy": 1, "fontSize": 16, "fontWeight": "bold", "color": "#5B2C83"},
                "encoding": {
...
def render_radar_like_chart(result: BriefingResult):
    ...
    for row in rows:
        radar_rows.append({"Dimenzió": row["Dimenzió"], "Csapat": "KTE", "Érték": row["KTE"], "order": order})
        radar_rows.append({"Dimenzió": row["Dimenzió"], "Csapat": "ELL", "Érték": row["ELL"], "order": order})
        order += 1
    ...
    spec = {
        ...
        "encoding": {
            "theta": {"field": "Dimenzió", "type": "nominal", "sort": None},
