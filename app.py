...
def extract_first_numeric_from_label(df, labels: List[str]) -> float:
...
def extract_second_numeric_from_label(df, labels: List[str]) -> float:
    for _, row in df.iterrows():
        row_vals = [str(v) for v in row.tolist()]
        row_text = " | ".join(row_vals).lower()
        if any(label in row_text for label in labels):
...
def extract_pct_from_label(df, labels: List[str]) -> float:
    for _, row in df.iterrows():
        row_vals = [str(v) for v in row.tolist()]
        row_text = " | ".join(row_vals).lower()
        if any(label in row_text for label in labels):
...
def extract_mmss_from_label(df, labels: List[str]) -> float:
    for _, row in df.iterrows():
        row_vals = [str(v) for v in row.tolist()]
        row_text = " | ".join(row_vals).lower()
        if any(label in row_text for label in labels):
...
def score_dimensions(metrics: Dict[str, float]) -> Dict[str, float]:
    pressing = (
        normalize_inverse(metrics.get("ppda", 8), 1, 8) * 0.35
        + normalize_direct(metrics.get("pressing_success_pct", 45), 20, 80) * 0.25
        + normalize_direct(metrics.get("high_pressing_success_pct", 40), 10, 85) * 0.20
        + normalize_direct(metrics.get("challenge_intensity_index", 12), 8, 18) * 0.20
    )
    buildup = (
        normalize_direct(metrics.get("passes_accurate_pct", 70), 55, 85) * 0.30
        + normalize_direct(metrics.get("short_pass_acc_pct", 70), 45, 90) * 0.15
        + normalize_direct(metrics.get("medium_pass_acc_pct", 72), 50, 85) * 0.15
        + normalize_direct(metrics.get("passes_open_play_acc_pct", 68), 50, 82) * 0.20
        + normalize_inverse(metrics.get("lost_balls_own_half", 12), 5, 25) * 0.20
    )
    transition = (
        normalize_direct(metrics.get("counter_attacks_with_shots_pct", 10), 0, 35) * 0.35
        + normalize_direct(metrics.get("counter_attacks", 10), 0, 30) * 0.20
        + normalize_direct(metrics.get("entries_box", 12), 5, 30) * 0.20
        + normalize_direct(metrics.get("avg_pass_length", 20), 15, 30) * 0.10
        + normalize_direct(metrics.get("attacks_with_shots_pct", 8), 3, 18) * 0.15
    )
    attack = (
        normalize_direct(metrics.get("positional_attacks", 60), 20, 90) * 0.20
        + normalize_direct(metrics.get("positional_attacks_with_shots_pct", 6), 1, 15) * 0.20
        + normalize_direct(metrics.get("passes_into_final_third", 120), 40, 220) * 0.20
        + normalize_direct(metrics.get("passes_into_box", 25), 5, 60) * 0.25
        + normalize_direct(metrics.get("key_passes", 4), 0, 15) * 0.15
    )
    defense = (
        normalize_inverse(metrics.get("xg", 1.2), 0.3, 2.0) * 0.15
        + normalize_inverse(metrics.get("shots", 10), 4, 20) * 0.20
        + normalize_inverse(metrics.get("shots_on_target", 4), 1, 8) * 0.20
        + normalize_inverse(metrics.get("passes_into_box", 25), 5, 60) * 0.20
        + normalize_inverse(metrics.get("lost_balls_own_half", 12), 5, 25) * 0.10
        + normalize_direct(metrics.get("defensive_duels_success_pct", 52), 35, 70) * 0.15
    )
    setpieces = (
        normalize_direct(metrics.get("set_piece_attacks", 10), 0, 25) * 0.25
        + normalize_direct(metrics.get("set_piece_attacks_with_shots_pct", 20), 0, 50) * 0.30
        + normalize_direct(metrics.get("corners", 5), 0, 12) * 0.15
        + normalize_direct(metrics.get("corners_with_shots_pct", 25), 0, 80) * 0.20
        + normalize_direct(metrics.get("free_kick_combinations_with_shots_pct", 15), 0, 50) * 0.10
    )
    physical = (
        normalize_direct(metrics.get("challenge_intensity_index", 12), 8, 18) * 0.25
        + normalize_direct(metrics.get("challenges_total", 200), 120, 260) * 0.20
        + normalize_direct(metrics.get("challenges_success_pct", 50), 35, 65) * 0.15
        + normalize_direct(metrics.get("air_duels_success_pct", 50), 30, 70) * 0.15
        + normalize_direct(metrics.get("ground_duels_success_pct", 50), 35, 65) * 0.10
        + normalize_direct(metrics.get("tackles_success_pct", 55), 35, 75) * 0.10
        + normalize_direct(metrics.get("dribbles_success_pct", 55), 35, 75) * 0.05
    )
...
def generate_profiles(
    ...
    opp_form = opp_ctx.get("formation_start", "3-5-2 / 3-4-3")
    team_form = team_ctx.get("formation_start", "3-5-2")
...
def run_briefing_engine(opponent_name: str, files: UploadedFiles) -> BriefingResult:
    ...
    if not team_metrics:
        team_metrics = {
            "ppda": 2.8,
            "pressing_success_pct": 47,
            "high_pressing_success_pct": 54,
            "passes_accurate_pct": 74,
            "short_pass_acc_pct": 73,
            "medium_pass_acc_pct": 79,
            "passes_open_play_acc_pct": 73,
            "lost_balls_own_half": 12,
            "counter_attacks": 15,
            "counter_attacks_with_shots_pct": 13,
            "entries_final_quarter": 38,
            "entries_box": 10,
            "attacks_with_shots_pct": 7,
            "avg_pass_length": 21.2,
            "positional_attacks": 71,
            "positional_attacks_with_shots_pct": 4,
            "passes_into_final_third": 155,
            "passes_into_box": 26,
            "key_passes": 4,
            "final_third_possession_pct": 23,
            "xg": 0.62,
            "shots": 8,
            "shots_on_target": 3,
            "defensive_duels_success_pct": 52,
            "set_piece_attacks": 8,
            "set_piece_attacks_with_shots_pct": 25,
            "corners": 3,
            "corners_with_shots_pct": 33,
            "free_kick_combinations_with_shots_pct": 25,
            "challenge_intensity_index": 11.6,
            "challenges_total": 203,
            "challenges_success_pct": 51,
            "air_duels_success_pct": 61,
            "ground_duels_success_pct": 47,
            "tackles_success_pct": 42,
            "dribbles_success_pct": 69,
        }

    if not opp_metrics:
        opp_metrics = {
            "ppda": 3.4,
            "pressing_success_pct": 56,
            "high_pressing_success_pct": 67,
            "passes_accurate_pct": 69,
            "short_pass_acc_pct": 59,
            "medium_pass_acc_pct": 72,
            "passes_open_play_acc_pct": 66,
            "lost_balls_own_half": 10,
            "counter_attacks": 24,
            "counter_attacks_with_shots_pct": 13,
            "entries_final_quarter": 43,
            "entries_box": 19,
            "attacks_with_shots_pct": 8,
            "avg_pass_length": 21.2,
            "positional_attacks": 60,
            "positional_attacks_with_shots_pct": 3,
            "passes_into_final_third": 138,
            "passes_into_box": 27,
            "key_passes": 4,
            "final_third_possession_pct": 20,
            "xg": 1.03,
            "shots": 8,
            "shots_on_target": 4,
            "defensive_duels_success_pct": 59,
            "set_piece_attacks": 17,
            "set_piece_attacks_with_shots_pct": 18,
            "corners": 4,
            "corners_with_shots_pct": 25,
            "free_kick_combinations_with_shots_pct": 22,
            "challenge_intensity_index": 12.3,
            "challenges_total": 235,
            "challenges_success_pct": 54,
            "air_duels_success_pct": 55,
            "ground_duels_success_pct": 54,
            "tackles_success_pct": 60,
            "dribbles_success_pct": 51,
        }
        ...
        rows.append(
            {
                ...
                "marker_size": 1400 if code in [selected_a, selected_b] else 1000,
...
def render_strategy_map(selected_a: Optional[str] = None, selected_b: Optional[str] = None):
    ...
    spec = {
        ...
        "layer": [
            {
                "mark": {"type": "circle", "opacity": 0.98, "stroke": "#5B2C83", "strokeWidth": 2.2},
                "encoding": {
                    "x": {
                        "field": "x",
                        "type": "quantitative",
                    ...
                    "color": {
                        "field": "marker_type",
                        "type": "nominal",
                        "scale": {"domain": ["Paletta", "Plan A", "Plan B"], "range": ["#FFFFFF", "#F3D34A", "#63D2C6"]},
            ...
            {
                "mark": {"type": "text", "dy": 2, "fontSize": 16, "fontWeight": "bold", "color": "#5B2C83"},
                "encoding": {
...
def render_radar_like_chart(result: BriefingResult):
    ...
    def polygon_points(values: List[float]) -> str:
        pts = []
        n = len(values)
    ...
    for i, label in enumerate(labels):
        ...
        lx = cx + math.cos(ang) * (max_r + 28)
        ly = cy + math.sin(ang) * (max_r + 28)
        anchor = "middle"
        if lx < cx - 20:
            anchor = "end"
        elif lx > cx + 20:
    ...
    kte_poly = polygon_points(kte_vals)
    ell_poly = polygon_points(ell_vals)
    ...
    <svg width="{size}" height="{size}" viewBox="0 0 {size} {size}" xmlns="http://www.w3.org/2000/svg">
      ...
      {''.join(grid_polys)}
      {''.join(axes)}
      <polygon points="{ell_poly}" fill="rgba(183,163,201,0.25)" stroke="#B7A3C9" stroke-width="3" />
      <polygon points="{kte_poly}" fill="rgba(91,44,131,0.18)" stroke="#5B2C83" stroke-width="3" />
      {''.join(label_svg)}
      <circle cx="405" cy="455" r="7" fill="#5B2C83" />
      <text x="420" y="460" font-size="14" fill="#2F1D4A">KTE</text>
      <circle cx="465" cy="455" r="7" fill="#B7A3C9" />
      <text x="480" y="460" font-size="14" fill="#2F1D4A">ELL</text>
    </svg>
    """
...
if pd is None or pdfplumber is None:
    missing = []
    if pd is None:
    ...
    st.warning(
        "A valódi Excel/PDF beolvasáshoz még hiányzik: "
        + ", ".join(missing)
        + ". Add hozzá a requirements.txt-hez: streamlit, pandas, pdfplumber, openpyxl"
    )
...
if step == "1. Input":
    ...
    if result is None:
        ...
        st.info(
            "Az arány nem szavazás. A Plan A az alap játékmodell, amely várhatóan a mérkőzés nagyobb részében érvényesül. "
            "A Plan B az alkalmazkodó viselkedés. A százalék a két modell várható meccsbeli megoszlását mutatja."
        )
